# C++ Text-to-Speech Makefile
# Framework-agnostic commands for managing the project and git submodules

.PHONY: help check check-prereqs init install install-backend build start start-backend start-frontend test update clean status eject-frontend

# Default target: show help
help:
	@echo "C++ Text-to-Speech - Available Commands"
	@echo "========================================"
	@echo ""
	@echo "Setup:"
	...
	@echo "  make install-backend  Configure and build C++ backend"
	@echo ""
	@echo "Development:"
...
# Build C++ backend: cmake configure + build
install-backend:
	@echo "==> Configuring and building C++ backend..."
...
start-backend:
	...
	@if [ ! -f "build/cpp-text-to-speech" ]; then \
		echo "Error: Backend not built. Run 'make install-backend' first."; \
		exit 1; \
	fi
	@echo "==> Starting backend on http://localhost:8081"
	./build/cpp-text-to-speech
