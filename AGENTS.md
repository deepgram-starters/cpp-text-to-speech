...
C++ (Crow) demo app for Deepgram Text-to-Speech.
...
| File | Purpose |
|------|---------|
| `src/main.cpp` | Main backend — API endpoints and request handlers |
...
```
feat(cpp-text-to-speech): add diarization support
fix(cpp-text-to-speech): resolve WebSocket close handling
refactor(cpp-text-to-speech): simplify session endpoint
